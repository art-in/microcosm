...
describe('middlewares', () => {
  ...
  it(`should emit 'mutation' events with unique mutation IDs`, async () => {
    ...
    expect(firstCallArgs[1].mutationId).to.not.equal(
      secondCallArgs[1].mutationId
    );

    expect(firstCallArgs[1].mutationId).to.equal(thirdCallArgs[1].mutationId);

    expect(secondCallArgs[1].mutationId).to.equal(fourthCallArgs[1].mutationId);
  });

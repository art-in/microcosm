...
export default function getNewIdeaPosition(
    ...
    const isFreePos = isFreePosition.bind(
        null,
        occupiedPositions,
        minDistance);
    ...
    while (!foundPos) {
        ...
        for (; relRad < HALF_CIRCLE_RAD; relRad += ANGLE_STEP_RAD) {
            ...
            if ((foundPos = isFreePos(currentRadius, angleRad1)) ||
...
function isFreePosition(
    ...
    if (occupiedPositions.length) {
        ...
        const farEnoughFromOccupiedPositions =
            occupiedPositions.every(occupiedPos => {
